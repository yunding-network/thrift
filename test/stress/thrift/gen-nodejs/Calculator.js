...
var Calculator_cal_args = function(args) {
  this.logid = null;
  this.w = null;
  if (args) {
...
Calculator_cal_args.prototype = {};
Calculator_cal_args.prototype.read = function(input) {
  input.readStructBegin();
  while (true)
  {
    var ret = input.readFieldBegin();
...
Calculator_cal_args.prototype.write = function(output) {
  output.writeStructBegin('Calculator_cal_args');
...
var Calculator_cal_result = function(args) {
  this.success = null;
  this.ouch = null;
...
Calculator_cal_result.prototype = {};
Calculator_cal_result.prototype.read = function(input) {
  input.readStructBegin();
  while (true)
  {
    var ret = input.readFieldBegin();
...
Calculator_cal_result.prototype.write = function(output) {
  output.writeStructBegin('Calculator_cal_result');
  if (this.success !== null && this.success !== undefined) {
    output.writeFieldBegin('success', Thrift.Type.I32, 0);
    output.writeI32(this.success);
    output.writeFieldEnd();
...
CalculatorClient.prototype.cal = function(logid, w, callback) {
  this._seqid = this.new_seqid();
  if (callback === undefined) {
    var _defer = Q.defer();
    this._reqs[this.seqid()] = function(error, result) {
...
CalculatorClient.prototype.send_cal = function(logid, w) {
  var output = new this.pClass(this.output);
  output.writeMessageBegin('cal', Thrift.MessageType.CALL, this.seqid());
  var args = new Calculator_cal_args();
  args.logid = logid;
  args.w = w;
  args.write(output);
  output.writeMessageEnd();
...
CalculatorClient.prototype.recv_cal = function(input,mtype,rseqid) {
  var callback = this._reqs[rseqid] || function() {};
  delete this._reqs[rseqid];
  if (mtype == Thrift.MessageType.EXCEPTION) {
    var x = new Thrift.TApplicationException();
  ...
  var result = new Calculator_cal_result();
  result.read(input);
  input.readMessageEnd();
  ...
  return callback('cal failed: unknown result');
};
...
CalculatorProcessor.prototype.process_cal = function(seqid, input, output) {
  var args = new Calculator_cal_args();
  args.read(input);
  input.readMessageEnd();
  if (this._handler.cal.length === 2) {
    Q.fcall(this._handler.cal, args.logid, args.w)
      .then(function(result) {
        var result_obj = new Calculator_cal_result({success: result});
        output.writeMessageBegin("cal", Thrift.MessageType.REPLY, seqid);
        result_obj.write(output);
        output.writeMessageEnd();
        output.flush();
      }, function (err) {
        ...
        if (err instanceof ttypes.InvalidOperation) {
          result = new Calculator_cal_result(err);
          output.writeMessageBegin("cal", Thrift.MessageType.REPLY, seqid);
        } else {
          result = new Thrift.TApplicationException(Thrift.TApplicationExceptionType.UNKNOWN, err.message);
          output.writeMessageBegin("cal", Thrift.MessageType.EXCEPTION, seqid);
        }
        result.write(output);
        output.writeMessageEnd();
        output.flush();
    ...
    this._handler.cal(args.logid, args.w, function (err, result) {
      var result_obj;
      if ((err === null || typeof err === 'undefined') || err instanceof ttypes.InvalidOperation) {
        result_obj = new Calculator_cal_result((err !== null || typeof err === 'undefined') ? err : {success: result});
        output.writeMessageBegin("cal", Thrift.MessageType.REPLY, seqid);
      } else {
        result_obj = new Thrift.TApplicationException(Thrift.TApplicationExceptionType.UNKNOWN, err.message);
        output.writeMessageBegin("cal", Thrift.MessageType.EXCEPTION, seqid);
