 ...
 * [ZCServer 构造函数]
 */
var ZCServer = function(prefix){
	if(prefix) this.prefix = prefix;
};
// 继承ZServer
util.inherits(ZCServer, ZServer);
 ...
 */
ZCServer.prototype.registerZookeeper = function(cb){
	var self = this;
	var services = _.keys(this.services);
	async.each(services, function(serviceName, next){
		var options = {
			connection_type: self.CONNECT_TYPE.COMMON
		};
